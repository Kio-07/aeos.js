...
/**
 * @param  {import('./classes/aeosClient.js')} client
...
const Interpreter = async (
    ...
    try {
        ...
        if (client?.aeosOptions?.debugs?.interpreter) {
            console.log(`|------------------------------------------|`);
            ...
            code = (
                await IF({
                    ...
                    data: {
                        ...
                        args: args,
                        aeosError: require("./classes/aeosError.js"),
                        data: data,
                        func: undefined,
                        funcLine: undefined,
                        util: Util,
                            ...
                            if (typeof unpacked.inside !== "string") {
                                if (suppressErrors) return suppressErrors;
                                    ...
                                    return client.options.suppressAllErrors
                                        ...
                                        : `\`aeosError: ${this.func}: Invalid Usage (line : ${funcLine})\``;
                                }
                            } else return false;
                        },
                        noop() {
                        ...
                        inside(unpacked) {
                            if (typeof unpacked.inside !== "string") {
                                if (suppressErrors) return suppressErrors;
                                    ...
                                    return client.options.suppressAllErrors
                                        ? client.options.errorMessage
                                        : `\`aeosError: ${func}: Invalid Usage (line : ${funcLine})\``;
                                }
                            } else return false;
                        },
                        noop() {
                            ...
                            if (client.options.suppressAllErrors) {
                                ...
                                if (suppressErrors && !errorOccurred) {
                                    if (suppressErrors.trim() !== "") {
                                      const { makeMessageError } = require("./classes/aeosError.js")
                                      const msg  = 
        ...
        if (embeds?.some((x) => x === undefined)) {
            error = true;
            return aeosError.consoleError("EmbedError", "Input of index's are empty");
        }
        if (returnCode) {
